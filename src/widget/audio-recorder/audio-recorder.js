...
class AudioRecorder extends Widget {
    ...
    _init() {
        ...
        this._handlePlayackPausedBound = () => this._handleEvent(AudioRecorder._EVENTS.PLAYBACK_STARTED);
        ...
        fragment.appendChild(this.resetButtonHtml);
        this._audioInstanceEl = fragment.querySelector(`.${AudioRecorder._CLASSES.AUDIO}`);
        this._audioRecorderBodyEl = fragment.querySelector(`.${AudioRecorder._CLASSES.BODY}`);
        this._resetButtonEl = fragment.querySelector(`.${AudioRecorder._CLASSES.RESET}`);
        ...
        if (
            ...
            fileManager.init()
                .then(() => {
                    this.enable();
                    this._audioRecorderBodyEl.addEventListener('click', () => this._handleEvent(AudioRecorder._EVENTS.WIDGET_BODY_CLICKED));
    ...
    _handleEvent(event, data) {
        ...
        switch (this._currentState) {
            ...
            case AudioRecorder._STATES.PLAYING_BACK_RECORDING:
                ...
                if (event === AudioRecorder._EVENTS.PLAYBACK_FINISHED) {
                    this._currentState = AudioRecorder._STATES.FINISHED_RECORDING;
                    this._setMessage(AudioRecorder._stateToMessage(this._currentState));
                    return;
                }
    ...
    _setMessage(message, severity) {
        ...
        this._audioRecorderBodyEl.innerHTML = messageTemplate;
    }

    disable() {
        this._audioRecorderBodyEl.disabled = true;
        this._audioRecorderBodyEl.classList.add('disabled');
        this._resetButtonEl.disabled = true;
        this._resetButtonEl.classList.add('disabled');
    }

    enable() {
        this._audioRecorderBodyEl.disabled = false;
        this._audioRecorderBodyEl.classList.remove('disabled');
...
AudioRecorder._CLASSES = Object.freeze({
    WIDGET: 'audio-recorder-widget',
    BODY: 'audio-recorder__body',
    RESET: 'btn-reset',
...
AudioRecorder._WIDGET_TEMPLATE = `
    <div class="widget ${AudioRecorder._CLASSES.WIDGET}">
        <button
            disabled
            type="button"
            class="${AudioRecorder._CLASSES.BODY} btn">
        </button>
        <audio
